...
model_kwargs={'loss_name':'nll', 'use_mask': True, 'hidden_sizes':(3,), 'activation':'elu', 'layer':tf.keras.layers.Dense}
NFEATS=2
...
def make_animation(features, targets, checkpoint_path, func, valpath, features_test, features_normer, targets_normer):
    ...
    for i,f in enumerate(iterlist):
        ...
        if float(tf.__version__[:3]) >2.0:
            test_pred = model([features_test, tf.constant(0, shape=features_test.shape), tf.constant(0, shape=features_test.shape)])
        elif float(tf.__version__[:3]) <=2.0:
            test_pred = model(features_test.astype('float32'))
        test_preds = test_pred.mean().numpy()
        test_preds_variance = test_pred.variance().numpy()
        ...
        fprima=1./targets_normer.b
        if targets_normer.type=="-11": fprima*=2
        test_preds_variance=np.ma.array(test_preds_variance, mask=~mask_test)/(fprima**2)
        #test_preds_variance=np.ma.array(test_preds_variance, mask=~mask_test)*(fprima**2)
        ...
        x2a=test_preds[:,0,0]
        x2aerr=test_val_biases_msb_err[:,0]
