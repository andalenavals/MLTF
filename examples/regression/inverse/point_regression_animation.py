...
def validate(features, targets, checkpoint_path, valpath):
    ...
    for f in checkpoints:
        ...
        loss_val=ML_Tensorflow.loss_functions.msb(tf.convert_to_tensor(targets.astype('float32')), tf.convert_to_tensor(preds.astype('float32')), mask=tf.convert_to_tensor((mask*1.0).astype('float32')))
        print(loss_val_direct,loss_val.numpy(), float("0.%s"%(f.split(".")[2])), float("0.%s"%(f.split(".")[2]))-loss_val_direct )
...
def test(features, targets, checkpoint_path, func, path, features_test):
    ...
    features_1d = np.ma.concatenate(features[:,:,0].T)
...
def make_animation(features, targets, checkpoint_path, func, valpath, features_test, features_normer, targets_normer):
    ...
    for f in iterlist:
        ...
        loss_val_direct=np.ma.mean(np.square(val_biases_msb))    
        loss_val=ML_Tensorflow.loss_functions.msb(tf.convert_to_tensor(targets.astype('float32')), tf.convert_to_tensor(preds.astype('float32')), mask=tf.convert_to_tensor((mask*1.0).astype('float32')))
        print(loss_val_direct,loss_val.numpy(), float("0.%s"%(f.split(".")[2])),  )
        ...
        make_plot(x1,y1, x2a,y2a,x2b,y2b, func, plotname=filename, ylim=[-0.04,0.05], title1="Loss: %.2e"%(float("0.%s"%(f.split(".")[2]))), title2="Epoch: %s"%(f.split(".")[1]).replace("-0","") )#, vmin=0, vmax=0.015, ylim=[-0.1,0.1])
...
def main():    
    ...
    #validate(features, targets, checkpoint_path, validationpath )
